const app = new Vue({
  ...
  data: {
    ...
    isMain: true,
    genres: [],
    url: "https://project-10-dongjunhyun.c9users.io/api/v1",
  ...
  methods: {
    toggleDetail: function (id = null) {  // 목록 <=> 상세 화면을 전환합니다. 인자로 id 가 들어올 경우, 상세화면을 표시합니다.
      if (id) {
        ...
        this.movieDetail.score_id = []
        this.movieDetail.scores = movie.scores  // 상세 페이지에서 표시할 모든 $data.scores 를 받아와야 합니다.
        this.movieDetail.genre = '' // 해당 movie 의 genreId 를 통해 genre.name 을 찾아서 저장합니다.
      }
      this.detailView = !this.detailView
    },
    writeScore: function (movie_id) {
      console.log(this.content, this.score, movie_id)
      axios.post(this.url+"/movies/"+movie_id+"/scores/", { content: this.content, score: this.score, movie: movie_id })
        .then(response => {
          // this.getMethod()
          getMethod(movie_id)
          this.content = ''
    ...
    getMethod: function(movie_id){
      axios.get(this.url+"/movies/"+movie_id)
      .then(response => {
        this.score_id += responase.data
      })
    },
  },
  computed: {
    filteredMovies: function () {
      if (!this.search) {//''<=사용자가 검색을 안함
        return this.movies
      }
      const search = this.search.trim().toLowerCase()
      //callback 함수에서 반환되는 값이 true인 아이템 만으로 새로운 배열 생성
      const newMovies = this.movies.filter(movie => {
        return movie.title.toLowerCase().trim().includes(search)
      })
      return newMovies
    }
  },
  ...
  created: function () {  // Vue 인스턴스가 생성되는 시점에 실행되는 함수입니다. 현재는 Vue 인스턴스가 생성되면, json-server 에서 장르목록만 받아옵니다.
    axios.get(`${this.url}/genres/`)  // 만약 json-server 를 사용하지 않거나 서버가 꺼져있다면 에러가 발생합니다.
      .then(res => this.genres = res.data)

    axios.get(`${this.url}/check/`)  // 만약 json-server 를 사용하지 않거나 서버가 꺼져있다면 에러가 발생합니다.
